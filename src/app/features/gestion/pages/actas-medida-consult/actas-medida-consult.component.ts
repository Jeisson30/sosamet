...
@Component({
  selector: 'app-actas-medida-consult',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './actas-medida-consult.component.html',
  styleUrls: ['./actas-medida-consult.component.scss'],
})
export class ActasMedidaConsultComponent implements OnInit {
  buscar = '';
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          Swal.fire(
            ...
            err?.error?.mensaje ||
              err?.error?.error ||
              'No se pudieron consultar las actas de medida.',
            'error'
          );
        },
      });
  ...
  onEditar(row: ActaMedidaHeader): void {
    if (this.isAnulada(row)) {
      Swal.fire(
        'Acta anulada',
        'No se puede editar un acta de medida anulada.',
        'warning'
      );
      return;
    }
    if (!this.puedeEditar) {
      Swal.fire(
        'Sin permiso',
        'Solo un administrador puede editar actas de medida.',
        'warning'
      );
      return;
    }
  ...
  onEliminar(row: ActaMedidaHeader): void {
    if (!this.puedeEditar) {
      Swal.fire(
        'Sin permiso',
        'Solo un administrador puede eliminar actas de medida.',
        'warning'
      );
      return;
    }
    ...
    Swal.fire({
      icon: 'warning',
      title: 'Eliminar acta de medida',
      html: `
        <p>Se eliminará el acta <strong>${consecutivo}</strong>.</p>
      ...
      this.contractsService.deleteActaMedida(consecutivo).subscribe({
        ...
        error: (err) => {
          Swal.fire(
            ...
            err?.error?.mensaje ||
              err?.error?.error ||
              'Ocurrió un error al eliminar el acta de medida.',
            'error'
          );
        },
      });
  ...
  onAnular(row: ActaMedidaHeader): void {
    if (!this.puedeEditar) {
      Swal.fire(
        'Sin permiso',
        'Solo un administrador puede anular actas de medida.',
        'warning'
      );
      return;
    }
    ...
    if (this.isAnulada(row)) {
      Swal.fire(
        'Ya anulada',
        'Esta acta de medida ya se encuentra anulada.',
        'info'
      );
      return;
    }
    ...
    Swal.fire({
      icon: 'warning',
      title: 'Anular acta de medida',
      html: `
        <p>Se anulará el acta <strong>${consecutivo}</strong>.</p>
      ...
      this.contractsService.anularActaMedida(consecutivo).subscribe({
        ...
        error: (err) => {
          Swal.fire(
            ...
            err?.error?.mensaje ||
              err?.error?.error ||
              'Ocurrió un error al anular el acta de medida.',
            'error'
          );
        },
      });
  ...
  actualizarActa(): void {
    ...
    if (!this.puedeEditar) {
      Swal.fire(
        'Sin permiso',
        'Solo un administrador puede actualizar actas de medida.',
        'warning'
      );
      return;
    }
